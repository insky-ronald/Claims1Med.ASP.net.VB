...
Public Class DataProvider
	...
	Private DBClinic As System.Data.DataTable
	...
	Protected Overrides Sub InitCallback(ByVal Action As String, ByVal Output As EasyStringDictionary)
		...
		ClinicID = Request.Params("keyid")
		DBClinic = DBConnections("DBMedics").OpenData("GetClinics", {"id","visit_id"}, {ClinicID, Session("VisitorID")}, "")

		If Action = "navigator"
			Output.AsString("page_title") = DBClinic.Eval("Clinic: @name")
			Output.AsString("window_title") = DBClinic.Eval("@name")

			CustomData.AsJson("hospital_id") = ClinicID
			CustomData.AsJson("data") = DBClinic.AsJson()
		...
		Else If Action = "refresh"
			Output.AsJson("data") = DBClinic.AsJson()
		End if
	End Sub
	...
	Protected Overrides Sub InitMenuItems(ByVal MenuItems As Navigator.MenuItems)
		...
		MenuItems.Params.AsInteger("hospital_id") = ClinicID
		...
		Dim Main As Navigator.MenuItem = MenuItems.AddMain("hospital", "Hospital")
			...
			If ClinicID <> 0
				With Main.SubItems.Add
					...
					.URL = "app/addresses"
					.Params.AsInteger("name_id") = ClinicID '<<-- add param here
				End with
				...
				With Main.SubItems.Add
					...
					.Action = "admin"				
					.URL = "app/contacts"
					.Params.AsInteger("name_id") = ClinicID
				End with
