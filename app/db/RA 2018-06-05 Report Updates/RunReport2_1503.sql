...
/****** Object:  StoredProcedure [dbo].[RunReport2_1503]    Script Date: 6/19/2018 1:05:51 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[RunReport2_1503] 
(
	@id int = 0, 
...
BEGIN
	...
	SET @source = 'v_report_1503'
	SET @total_source = ''
	SET @source_join = 'id'
	SET @final_source = 'v_report_1503'
	SET @totals = ''
	SET @columns = '*'
	...
	--Case Fee Status
	EXEC RunDynamicQueryBuilder2 @id=@id, @type='c', @operator='=', @name='case_fee_status_code', @column_name='invoice_status_code', @where = @where OUTPUT, @parameters = @parameters OUTPUT, @values = @values OUTPUT
	EXEC RunDynamicQueryBuilder2 @id=@id, @type='c', @operator='=', @name='case_fee_sub_status', @column_name='invoice_sub_status_code', @where = @where OUTPUT, @parameters = @parameters OUTPUT, @values = @values OUTPUT
	--Dates
