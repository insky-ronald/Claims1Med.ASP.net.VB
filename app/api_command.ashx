...
Public Class Api
	...
	Protected Overrides Sub ReturnOutput(ByVal Cmd As String, ByVal Output As EasyStringDictionary)
		...
		If Cmd = "move-action-item"
			Using Command = DBConnections("DBSecure").PrepareCommand("MoveActionItem")
				Command.SetParameter("id", Request.Params("id"))
				Command.SetParameter("target_id", Request.Params("target_id"))
				Command.SetParameter("action", Request.Params("action"))
		...
		' If Cmd = "add-claim-diagnosis"
			' Using Command = DBConnections("DBMedics").PrepareCommand("AddClaimDiagnosis")
				...
				' Output.AsString("message") = "Test error..."
			' End Using
		' End if
		...
		If Cmd = "move-schedule-item"
			Using Command = DBConnections("DBMedics").PrepareCommand("MoveScheduleItem")
				Command.SetParameter("id", Request.Params("id"))
				Command.SetParameter("target_id", Request.Params("target_id"))
				Command.SetParameter("action", Request.Params("action"))
				Command.SetParameter("visit_id", VisitorID)
